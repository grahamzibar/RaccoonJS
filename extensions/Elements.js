(function __elements__() {
	...
	var init = function init() {
		if (window.EPE) {
			...
			// that's totally not cool... refresh the page?
			window.location.replace(window.location.href);
		}

		extensions.Elements = new (function Elements() {
			...
			// container is either an HTMElement or a dictionary of objects
	...
	if (navigator.appName == 'Microsoft Internet Explorer' && navigator.userAgent.indexOf('MSIE 9.0') == -1) {
		ScriptLoader.requireScript('extensions.Elements', 'utils.Browser', 'extensions.IE', init);
	} else {
		ScriptLoader.requireScript('extensions.Elements', 'utils.Browser', init);
	}
})();